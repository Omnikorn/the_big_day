...
const { User, wedding, Guests, Wedding } = require('../models');
...
for (let i = 0; i<weddingData.length; i++){
    const{_id, wedding_owner}=await Wedding.create(weddingData[i]);
...
for (let j=0; j<guestsData.length; j++){
    const {_id, wedding_owner} = await Guests.create(guestsData[j]);
    const user= await User.findOneAndUpdate(
        {username:wedding_owner},
        {
            $addToSet:{
